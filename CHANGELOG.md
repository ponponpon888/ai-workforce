...
- **ページを訳したのに、正本の README と CONTRIBUTING が「英語版は無い」と言い続けていた**のを
  直し、機械で縛った（[doc-003](data/pitfalls/doc-003.json)）。ページを訳すのは3箇所の編集になる
  （`.en.md` を置く / 日本語の原本の冒頭に `English:` の行 / 両方の README の表にリンク）。
  `docs/04` と `docs/08` を訳したあと、ファイルも `English:` 行も `README.en.md` の表も
  揃っていたのに、**`README.md`（正本）の表だけリンクが無かった**。`CONTRIBUTING` も
  **英語の節だけが直っていて、日本語側は「残りの `docs/`（04・08）の英訳は歓迎します」のまま**。
  正本のほうが間違っている状態だった。`README.en.md` の「英語版があるのは 00 と 02 の2本」も、
  9本になったあと残っていた。
- **`kit/scripts/check-translation-links.mjs` を追加した**（`docs` ワークフローから実行）。
  `docs/<base>.en.md` が存在するページそれぞれについて、日本語ページの冒頭（先頭10行以内）・
  `README.md`・`README.en.md` の3箇所がその対訳を張っていることを見る。対訳が無いページは
  どこからも要求しない（`README.en.md` から日本語ページへ張るのは、その場合に正しいこと）。
  **`check-doc-links` は逆向き**——無い翻訳へのリンク——を見ている。**在る翻訳が張られて
  いない**ほうは誰も見ていなかった。壊れていないからで、主張が不完全なだけでは目に見えない。
- **散文の列挙を3箇所から消した**（`CONTRIBUTING` の日英、`README.en.md` の「2本」）。
  どのページが訳されているかを文で書くと訳した瞬間に古くなり、**文をファイル一覧に縛る方法は
  新しい記法を作らない限り無い**。`doc-002` のときは `# pass: N` という既存の書き方が使えたが、
  ここには使えるものが無かった。だから表に答えを持たせ、表を検査する。
...
- **README がコマンドの隣に載せていた実行結果が古くなっていた**のを直し、同じずれを機械で縛った
  ...
  すぐ隣にその出力を書いている。誰も走らせていなかったので古くなった。実測時点で guard-sql は
  **91**、guard-config は **53** を出していたのに、両 README の**10箇所のうち6箇所**が
  87 と 45 のままだった。コマンドを実行した読者は、それを読んでいるページと違う数字を見る。
  [doc-001](data/pitfalls/doc-001.json) のひとつ外側で、あちらは散文がレコードのフィールドを、
  こちらはコマンドの出力を引用したまま古くなった。
- **`kit/scripts/check-doc-commands.mjs` を追加した**（`docs` と `test` の両方から実行）。
  フェンスの中の `node kit/scripts/test-<name>.mjs   # pass: N   fail: N` を見つけて、
  そのスイートを実際に走らせて突き合わせる。**新しい記法は作っていない。** この書き方は
  リポジトリが元から使っていたもので、検査はそれを実行するだけ。
  **実行するのは実在する `kit/scripts/test-*.mjs` だけで、引数を文書から取ることは決して
  しない**（ページに書いてあるものを走らせる検査は、ページを編集すれば何でも実行できる
  仕組みになる。テストの大半はこの一点に割いた）。スイートの出力は2種類あり
  （独自ランナーの `pass: N   fail: N` と `node:test` の TAP）、片方しか読めない検査は
  残り半分を黙って検査しなくなるので両方読む。
  `test.yml` にも入れたのは、テストを1件足しただけで、Markdown を1行も触らずに
  README が偽になるため。
- **合計値をコマンドの隣に一度だけ置くようにした。** 実測すると、壊れるのは合計値だけで、
  内訳（止める 42 / 通す 32 / 承認トークン 11 など）は今も正しかった。ケースを1件足すたびに
  変わるのが合計値なので当然で、だから走らせて照合できる場所に一度だけ書く。消したのは
  README の表の「テスト」列（10行下のフェンスと同じ数字の重複）、`docs/00` の
  「guard-sql 44 ケース・pull-all 23 ケース」（実測 91・73）、`docs/02` 日英の
  「合計 45 ケース」（実測 53）、`docs/13` の「26件」（実測 30）。
  `docs/02` と `docs/13` は合計をコマンドの隣へ移したので、いまは検査対象になっている。
- **CI がサポートの切れた Node 20 で全テストを回していた**のを直した
  （[ci-002](data/pitfalls/ci-002.json)）。`test.yml` の3ジョブと `docs.yml` が
  `node-version: 20` を渡していたが、**Node 20 は 2026-04-30 にサポート終了**していて
  （[nodejs/Release](https://github.com/nodejs/Release) の `schedule.json` で確認）、
  5ヶ月が過ぎていた。全スイートは緑だったが、**誰も走らせるべきでないバージョンでの緑**は
  このワークフローが出したい信号ではない。`docs/11` も「Node.js 20以上」と書いていたので
  両者は食い違ってすらおらず、ドリフト検査では見つからない。4箇所を 22 に上げ、
  `docs/11` の要件も 22 以上にした。22 は修正が続いている一番古い系列（2027-04-30 まで）で、
  手元の開発環境と `critical-gate` の Linux ジョブが元から使っていたバージョンでもある。
  **これで CI と手元の検証が同じランタイムになった。**
  なお #47 で actions 側を node24 対応版へ上げたが、あれは action の実行環境の話で、
  テストを走らせるランタイムは別だったため、こちらは残っていた。
  機械検査は足していない。**壊れているという事実がリポジトリの外（Node のリリース計画）に
  ある**ので、リポジトリ内の検査では原理的に検出できない。レコードの `detection` は
  `node-version: 20` への退行しか見ておらず、22 の終了には気づけない。
  フックに実行時のバージョン判定を足す案も採らなかった。フックは **exit 2 以外の非ゼロが
  「報告されるが何もしない」＝実質の許可**になるため、古い Node を弾こうとした判定が
  そのまま素通しの穴になる。
...
- `docs/04`（複数案件運用）と `docs/08`（落とし穴の機械可読化）の英訳を追加。これで
  ...
  `docs/04` 後半の「PowerShell 版の実行は未検証」「PowerShell は未実行」といった断り書きは
  1件も落としていない（日英とも5件、39 / 41 / 44 / 47 / 51 という実測の件数もそのまま）。
- 訳す前に `docs/08` の**古くなった件数を本文から外した**。「いま 12 件しか入っていません」
  「候補は 140 件」「lint-pitfalls で5件の検査」は、それぞれ実際には 26 件 / 15 件で、
  そのまま訳せば英語版にも偽の記述が入るところだった。件数は
  `data/pitfalls.index.json` が持っていて、生成物なのでレコードを足せば必ず合い、
  CI も差分で赤くする。だから散文には書かず、index を指す形にした。これは `docs/08` 自身が
  `measured_on` について書いていたこと（手書きの「最終確認日」は書いた翌日から嘘になるので
  計算する）と同じ話だった。
- あわせて、**書き換えてはいけない数字との区別**も書いた。[docs/17](docs/17-integration-status.md)
  のテスト件数も現在の実測と5件食い違っているが、あれは検証対象コミットと日付を明記した
