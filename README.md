...
最初に流したときに1つ見つかり、その場で直しました。**`guard-config` が守るのは「インストール
された場所」ではなく `~/.claude` でした。** 標準以外の場所へ入れると、フックは置かれて登録も
されるのに、実際に使われている設定は無防備。いまは自分の位置から導出します
（[hook-013](data/pitfalls/hook-013.json)）。
