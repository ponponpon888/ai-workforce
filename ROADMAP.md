...
## probe-guards（登録されたガードに実際に入力を渡す）— 実装済み

**2026-09-21、`doctor.mjs` が構造上どうしても埋められない「static-pass はフックが動いた意味では
ない」という穴に着手した。** `docs/09` はその穴を「無害な対照入力でフックが呼ばれることを
確認してください」と人手に投げていたので、そこを毎回同じ形で実行する `probe-guards.mjs` を足した。

`doctor.mjs` 側には入れていない。あのスクリプトは「設定に書かれたコマンドを実行しない」ことが
help・docs/09・テストで保証されている契約で、`--probe` を足すとその契約が壊れるため。

最初に流した時点で1件見つかった（[hook-013](data/pitfalls/hook-013.json)）。`guard-config` は
自分がインストールされた場所ではなく `~/.claude` を守るので、標準以外の場所へ入れると
「登録されているのに、実際に使われている設定は無防備」になる。塞ぎ方は分かっているが、
Node版とPowerShell版の両方と既存テスト（guard-config の45件など）の見直しを伴うので、
ガードを直すPRとして独立させる。それまでは probe-guards が実行時に検出して incomplete で報告する。

## hook-013（guard-config が守る場所）— 修正済み

**2026-09-21、probe-guards が最初に流した時点で見つけた1件を塞いだ。** `guard-config` は
保護対象を実行時に `AIWF_CLAUDE_HOME`（未設定なら `~/.claude`）から決めており、自分が
どこに置かれたかを見ていなかった。標準以外の場所へインストールすると、フックは登録されて
いるのに実際に使われている設定は無防備という、このキットが一番避けたい形になっていた。

自分の位置から導出する形にした。明示的な上書き（環境変数・パラメータ）はそのまま残し、
指定が無いときは導出した home と `~/.claude` の両方を守る。守る範囲を広げるのは安全だが、
黙って狭めるのは危険なので、チェックアウトから直接登録されたコピーが `~/.claude` を
守らなくなる事態は避けている。

テストは Node 版・PowerShell 版とも 45 → 50 件。増やした5件は修正前の実装で3件落ちる
（＝再現テストとして機能する）ことを確認済み。
