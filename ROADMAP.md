...
**いまの状態**: v0.1 に向けた修正の統合検証中です。
PR #9〜#15 の取り込み範囲と Linux / Node の検証結果は [統合状況](docs/17-integration-status.md) にまとめています。
既存設定へのマージ・復元・最小試行の文書、doctor、linter、文書マーカー検査はこのブランチにあります。
以下のチェック欄はリリースや実機確認を済ませた扱いにせず、残作業の履歴として保持しています。
Windows・CI・Claude Code 本体の統合確認は未完了です。
...
- [ ] **`kit` の `defaultMode: "ask"` は存在しない値です。**
      ...
      なく**、黙殺されるなら組み込みの既定に落ちます。Pro / Max / Team のターミナルでは
      その既定が `auto` なので、**「全部承認なしで通すモードに入れなくしています」と
      説明したまま auto で始まる**可能性があります。docs/02 は日英とも、docs/03 は
      実物についても `ask` と書いています（実物は `default` なので、そちらは端的に
      誤りです）。使い捨ての設定で 1 回測れば決まります
- [ ] **`guard-sql` は「SQL クライアントを呼ぶ文字列」を引数に持つだけのコマンドも
      止めます。** 2026-09-09、フックの動作確認をしようとして、確認用ペイロードを
      コマンドラインに載せた呼び出し自体がブロックされました。フックが生きている
      証拠ではありますが、**テストや検証のためにコマンド行を引用する操作が通らない**
      ということでもあります。避けるにはペイロードをファイルに置いて標準入力から
...
- [x] **`test-pull-all.mjs` の認証設定検査を実挙動へ置き換えました。**
      ローカル HTTP 401 と実際の Git を使い、認証ヘルパーへの設定伝達、
      待たずに失敗を記録すること、コミット・作業ツリーの保持を確認します。
      Linux / Node の確認と負の対照は成功。今回の Windows / PowerShell、
      GCM 本体の GUI 抑制は未検証です（[認証検査](docs/18-pull-auth-check.md)）。
