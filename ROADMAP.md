...
- [x] **`guard-sql` がファイル経由の SQL を 1 件も見ていなかったので塞ぎました（`hook-002`）。**
      ...
      書いてから流す 2 段で 3 層とも抜けました。**参照先を読んで中身を検査し、
      1 つも読めなければ DDL と同じ承認を要求する**形で塞ぎました。テストは
      29 件から 44 件に。きっかけは
      [他の人の突破テスト](https://qiita.com/dai_chi/items/8c351cf7812dd31b7767)。
      **移植した PowerShell 版はその日のうちに黙って素通りしていました**
      （`hook-003`。返り値のコレクションが展開されて `.Count` が落ち、
      fail open の catch が exit 0 にしていた）
- [x] **インストーラが `approve-ddl` を置いていませんでした（`install-001`）。**
      `guard-sql` は DDL を承認トークンが出るまで止めて、拒否メッセージで承認
      スクリプトの実行を指示します。ところが入っていたのは CLAUDE.md・フック・
      `settings.json` だけで、メッセージが名指しするのは
      `kit/scripts/approve-ddl.mjs` というリポジトリ相対のパスでした。
      **kit を入れた人の作業ディレクトリには存在しません。** 止められた人に
      できることが「フックを外す」しかない状態でした。
      `<claude home>/scripts/` に置き、フックは隣にあればその絶対パスを出します
- [x] **CI のインストーラのジョブが、終了コードしか見ていませんでした。**
      何も書かなくても緑になります。約束しているファイルを名指しで確認する
      形にしました。`-SkipSettings` / `-SkipClaudeMd` が実際に飛ばすことも見ます
- [x] **`-SkipClaudeMd` を足しました。** 自分の `CLAUDE.md` を持っている人が、
      設定だけ据え置いてフックを入れ替えられるようにするためです
      （バックアップは取られますが、取られることと望むことは別です）
...
- **落とし穴のレコードを増やす。** docs を読み直して列挙した候補は 140 件
      ...
      **候補リスト由来が 8 件だけ**です（レコード総数は 12 件。`hook-002`
      `hook-003` `install-001` `shell-001` は候補リストにない、あとから測った
      観測）。残り 132 件は 1 件も入っていません。形は
