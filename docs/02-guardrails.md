...
**2 つ目は、settings.json の書き方です**（[hook-009](../data/pitfalls/hook-009.json)）。
guard-secrets と guard-destructive を、matcher が同じ `Bash|PowerShell` の
**別々のエントリ**に書いていました。`/hooks` を開くと、`Bash|PowerShell` の下は
**1 hook** でした。同じ入力をフックに直接渡すと止まるので、フック本体は動いています。
2 つを 1 つのエントリにまとめて入れ直すと **2 hooks** になり、3 つとも止まりました。
doctor に、同じ matcher のエントリが 2 つあったらエラーにする検査を足しています。

どちらも、単体テストからは見えない場所でした。1 つ目は「どこに置かれるか」、
2 つ目は「Claude Code がどう読むか」です。**実機で `/hooks` を開いて件数を数える**のが、
一番安い確認でした。
