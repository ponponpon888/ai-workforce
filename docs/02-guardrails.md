...
[guard-destructive.ps1](../kit/claude/hooks/guard-destructive.ps1)。同じテストを
両方に当てています）は、deny に書いてある
破壊系のコマンドが、**deny の一致しない形で来たとき**に止めます。禁止する中身は
基本的に deny と同じです（deny に無いものは、あとで書く 4 つだけ足しました）。
...
### deny に無いものを、4 つだけ足しました

ここまでは「deny に書いてあるものを、deny が一致しない形でも止める」だけでした。
あとから、deny に対応する行がないものも 4 つ足しています。

| 足したもの | 何が起きるか |
|---|---|
| `find ... -delete` | `rm -rf` と同じことを、別の綴りでやる |
| `git stash drop` / `git stash clear` | コミットしていない変更を捨てる |
| `gh repo sync --force` | ローカルではなく**リモート**のブランチを上書きする |
| `gh repo delete` | リポジトリそのものを消す |

基準は「取り消せるか」です。deny に並んでいる破壊系コマンドと同じで、
消えたものが戻りません。

**`git checkout -- .` と `git restore` は、あえて入れていません。** 捨てるのが
未コミットの変更である点は `git stash drop` と同じですが、エージェントが自分で
生成したファイルを捨てる操作と区別がつきません。止めると、事故を防ぐ回数より
誤検知のほうが多くなります。必要になったら、引数が `.` のときだけ、のような形で
見直します。

### 通す側を多めに書きました

テストは 219 件で、止める側が 143 件、通す側が 76 件です（`--target ps` は 218 件。
差の 1 件は、リンク経由で起動するテストで、Node 版のファイルにしか当てられません）。
通す側で一番多いのは、
...
テストは全部通っていました。それでも Windows の Claude Code に
`cmd /c rd /s /q <存在しないフォルダ>` を打たせると、そのまま実行されました。
原因は 2 つ、別々にありました。

**1 つ目は、フック自身の起動判定です**（[hook-008](../data/pitfalls/hook-008.json)）。
テストから読み込めるように「自分が直接起動されたときだけ動く」判定を入れていました。
Node はシンボリックリンクを解決してからファイルを読むので、リンクを含む場所に置くと
この判定が外れ、フックは何も見ずに exit 0 します。一時ディレクトリが
`/var` → `/private/var` のリンクになっている macOS の CI だけが落ちて、気づきました。
判定は消して、リンク経由で起動するテストを足しています。

**2 つ目は、試していた設定が古かったことです**（[hook-009](../data/pitfalls/hook-009.json)）。
`/hooks` を開くと、`Bash|PowerShell` の下は **1 hook** でした。同じ入力をフックに直接渡すと
止まるので、フック本体は動いています。

最初は「guard-secrets と guard-destructive を、matcher が同じ `Bash|PowerShell` の別々の
エントリに書いたせいで、片方しか登録されなかった」と読みました。2 つを 1 つのエントリに
まとめる修正と、doctor の検査まで書きました。**これは間違いでした。**

インストーラが作るバックアップのファイル名には時刻が入っています。まとめた版を入れたのは
17:23 でしたが、3 つとも止まった確認はその前の 17:19 に終わっていました。その時点の
ディスクは、エントリを分けた版のままです。起動し直したら `/hooks` は **2 hooks** になり、
両方動いていました。失敗していたときの `/hooks` の中身も、入れ替え前の設定ファイルと
まったく同じ 3 つの matcher でした。

つまり、**起動中のセッションが、入れ替え前の設定のまま動いていた**だけです。入れ替え前の
設定も以前に入れたこのキットの版で、guard-config が入っていました。その版なら、guard-config は
起動中のセッションへの settings.json の変更の反映を拒否します
（[hook-007](../data/pitfalls/hook-007.json)。ConfigChange の登録があったかどうかは控えて
いません）。自分で入れた歯止めが、自分の確認を古い設定に閉じ込めていた可能性が高いです。doctor の検査は取り消し、インストーラの最後の案内に
「起動し直して、`/hooks` で 2 hooks になっているのを見てから試す」を足しています。

「記法が 2 つあって、どちらも効いていました」と同じ形の間違いです。**比べる前に、
何と何を比べているかを確かめていませんでした。**

どちらも、単体テストからは見えない場所でした。1 つ目は「どこに置かれるか」、
2 つ目は「いま、どの設定で動いているか」です。**試す前に `/hooks` を開いて、件数が
ファイルと合っているかを数える**のが、一番安い確認でした。
