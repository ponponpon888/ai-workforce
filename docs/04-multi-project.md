...
English: [04-multi-project.en.md](04-multi-project.en.md)
...
ログディレクトリの作成またはログ追記に失敗すると、エラーを表示して終了コード1で停止します。
最初のログに失敗した場合は、リポジトリの更新を開始しません。
更新開始後のログエラーでは、それ以降の処理を停止しますが、すでに完了した更新は巻き戻しません。
`--dry-run` / `-DryRun` はログを書かないため、この保存処理を実行しません。

## PowerShell のログ保持日数

`-LogRetentionDays` は1以上を指定します。0または負数は終了コード2で拒否し、
ログの作成・削除とリポジトリ更新を始めません。既定値は30日です。

## 現在のブランチを取得できない場合

コミットのない初期状態など、現在のブランチの取得がエラーになる場合は `fail/branch` と報告します。
そのリポジトリは取得・更新せず、残りの対象の処理を続け、全体の終了コードは1になります。
エラーメッセージをブランチ名として扱うことを防ぎます。
