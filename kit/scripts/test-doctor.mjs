...
try {
  ...
  test('PowerShell installer command format recognized', () => { const s = clone(); writeFileSync(join(temp,'hooks','lib','shell-lex.ps1'), '# not executed'); for (const n of ['guard-sql','guard-secrets','guard-config','guard-destructive']) { writeFileSync(join(temp,'hooks',n+'.ps1'), '# not executed'); hookOf(s, n).command = `powershell.exe -NoProfile -ExecutionPolicy Bypass -File "${join(temp,'hooks',n+'.ps1').replaceAll('\\','/')}"`; }
